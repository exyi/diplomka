...
class ConvEncoder(tf.Module):
    ...
    @tf.Module.with_name_scope
    def __call__(self, input: Union[tf.Tensor, tf.RaggedTensor], lengths: Optional[tf.Tensor] = None):
        x: Any = input
...
class Decoder(tf.Module):
    ...
    @tf.Module.with_name_scope
    def __call__(self, encoder_outputs: tf.RaggedTensor) -> tf.RaggedTensor:
...
class Network(tf.keras.Model):
    ...
    def call(self, input: TensorDict, whatever =None):
        ...
        one_hot_seq = tf.one_hot(input["sequence"], depth=len(csv_loader.basic_nucleotides), dtype=tf.float32)
        # tf.print("Sequence: ", input["sequence"])
        in_tensor = tf.concat([
            one_hot_seq,
            tf.cast(tf.expand_dims(input["is_dna"], -1), tf.float32)
        ], axis=-1)
        assert in_tensor.shape[-1] == Network.INPUT_SIZE
        assert in_tensor.shape[-2] == None

        encoder_output = in_tensor
    ...
    def ntcloss(self, target: tf.RaggedTensor, output: tf.RaggedTensor):
        target_onehot = tf.one_hot(target.values, depth=len(csv_loader.ntcs), dtype=tf.float32)
