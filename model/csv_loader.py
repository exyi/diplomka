...
basic_nucleotides = np.array([ "X", "A", "T", "U", "G", "C", " " ])
ntc_frequencies = {
    "NANT": 2738272,
    "AA00": 2880801,
    "AA08": 1253456,
...
def _process_subchain(steps: List[Any]):
    ...
    * is_dna - bool array
    ...
    if np.any(indices == ''):
        x = np.where(indices == '')[0]
        raise ValueError(f"Empty generated for steps {x}: {steps[x[0]].step_ID}")
...
def load_csv_file(file) -> Tuple[pd.DataFrame, PdbChainsDict]:
    ...
    * dictionary of chains: pdbid, chain -> dict
    ...
    table['pdbid_with_model'] = splitStepID[0]
    table['pdbid'] = [ x.split('-')[0] for x in splitStepID[0] ]
    table['model_i'] = [ [
        a := re.search(r"[a-zA-Z0-9]{4}-m(\d+)", x, re.IGNORECASE),
